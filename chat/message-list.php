...
while($row=$sql_ckecknotify->fetch()){
	if($row['uID']==$loggedin_id)//Current message was send by the user
	{
		...
		echo "<div  id='message-descriptor'  ><a href='chat/?id=$frndId' target='chatwindow'><b>".$name."</b></a><br>";
	...
	else//Current message was send by user's frnd
	{
		...
		echo "<div id='message-descriptor' class='unread-message'><a href='chat/?id=$frndId' target='chatwindow'><b>".$name."</b></a><br>";
		echo "▷ ".$row['description']."</div><br>";
		}
		else
		{
			echo "<div id='message-descriptor' class='read-message'><a href='chat/?id=$frndId' target='chatwindow'><b>".$name."</b></a><br>";
			echo "▷ ".$row['description']."</div><br>";
		}
	}
}
