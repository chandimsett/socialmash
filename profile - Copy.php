...
function showVote($objectId,$upcount,$downcount,$target_type)
{
	...
	if($checkUpvote==1)
			echo "<span title='upvote' class='upvote_$class_identifier' id='$objectId' data-target_type='$target_type'>▲</span><span class='upcount_$class_identifier' id='$objectId'>$upcount</span>";
	else
		    echo "<span title='upvote' class='upvote_$class_identifier' id='$objectId' data-target_type='$target_type'>△</span><span class='upcount_$class_identifier' id='$objectId'>$upcount</span>";
	if($checkDownvote==1)
			echo "<span title='downvote' class='downvote_$class_identifier' id='$objectId' data-target_type='$target_type'>▼</span><span class='downcount_$class_identifier' id='$objectId'>$downcount</span>";
	else
		    echo "<span title='downvote' class='downvote_$class_identifier' id='$objectId' data-target_type='$target_type'>▽</span><span class='downcount_$class_identifier' id='$objectId'>$downcount</span>";
	echo "</div>";	
...
function showPosts($uId)
{
	...
	while($row=mysqli_fetch_array($sql_posts))
	{
		...
		echo "<a href='#profile?id=";echo $posts->post_person->f_id; echo "' >";// echo $posts->post_person->f_name; echo "'>";
		echo $posts->post_person->f_name; echo "</a>";
		echo "<br>";
		echo "<small>".$posts->category."</small><br>";
		echo "$posts->content<br>";
		echo "<small>".$posts->timestamp."</small><br>";
		showVote($pId,$posts->upcount,$posts->downcount,"simple post");
		echo "<br>";
		//commenting
		echo "<textarea class='commentbox' id='commentbox-$pId' ></textarea>";
		...
		for($i=0;$i<count($comments_list->comments);$i++)
		{
			...
			showVote($cId,$comments_list->comments[$i]->upcount,$comments_list->comments[$i]->downcount,"simple comment");
			echo "<a href='#profile?id=".$comments_list->comments[$i]->commenter_person->f_id."'>"; echo $comments_list->comments[$i]->commenter_person->f_name;echo "</a>";
			echo $comments_list->comments[$i]->content;
			echo "<br>";
		}
